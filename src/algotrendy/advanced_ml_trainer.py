...
# Defer heavy ML/pandas imports to runtime inside methods to avoid import-time failures
...
class AdvancedMLTrainer:
    ...
    def load_and_prepare_data(self, period: str = "180d", interval: str = "5m",
        ...
        # Load data (import pandas lazily)
        try:
            import pandas as pd
        except Exception:
            pd = None
    ...
    def _advanced_feature_engineering(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Time features (guard access to index attributes)
        try:
        ...
        data['trend_strength'] = abs(data['close'] - data['close'].shift(20)) / data.get('atr', 1)
        try:
            import pandas as pd
