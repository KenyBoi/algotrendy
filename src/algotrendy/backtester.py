...
# Heavy data and plotting libraries are imported only where needed to keep
# module import lightweight for tests and tools.
...
class Backtester:
    ...
    def __init__(self, initial_capital: float = 100000.0,
        ...
        # Keep lightweight defaults here; initialize real Series in reset().
        self.equity_curve = []
        self.drawdown_curve = []
    ...
    def update_portfolio_value(self, date: datetime, prices: Dict[str, float]):
        ...
        try:
            ...
            if self.asset_type == "futures":
                ...
                # For stocks, positions are stored as dicts with 'quantity'
                for symbol, position_info in self.positions.items():
                    if symbol in prices and position_info and position_info.get('quantity', 0) != 0:
                        qty = position_info.get('quantity', 0)
                        position_value += abs(qty) * prices[symbol]
    ...
    def calculate_performance_metrics(self) -> PortfolioMetrics:
        ...
        try:
            ...
            # Local imports to avoid heavy module-level dependencies
            import numpy as np
